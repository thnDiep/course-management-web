...
export default {
  ...
  async getTopCategory(){
    ...
    for (const parent of categories) {
      ...
      const [numberCourses] =await this.getNumberEnrollCourseByCategory(parent.id);
      if(numberCourses!==undefined){
        let sum = +(numberCourses?.numberCourses);
        for (const child of childs) {
          const [numberCourse] = await this.getNumberEnrollCourseByCategory(child.id);
          if(numberCourse!==undefined)
            sum+= (+numberCourse?.numberCourses);
        }
        parent.numberCourses=sum
        if(topCategory.length===8)
          break;
